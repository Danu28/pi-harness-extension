...
import { readJsonStore, writeJsonStore } from "./store.mjs";
...
/** Load the persisted gate cache (newest-first, capped). */
export function loadGateCache(cwd) {
  return { entries: readJsonStore(cwd, GATE_CACHE_FILE, "entries").slice(0, GATE_CACHE_CAP) };
}

/** Best-effort persist of the gate cache. */
export function saveGateCache(cwd, entries) {
  writeJsonStore(cwd, GATE_CACHE_FILE, "entries", entries.slice(0, GATE_CACHE_CAP));
}
...
/** Load persisted red-gate rollback records (newest-first, capped). */
export function loadGateRollbacks(cwd, max = 20) {
  return readJsonStore(cwd, GATE_ROLLBACK_FILE, "records").slice(0, max);
}
...
export function recordGateFail(cwd, { head, verifyCmd, reason }) {
  try {
    ...
    writeJsonStore(cwd, GATE_ROLLBACK_FILE, "records", recs.slice(0, GATE_ROLLBACK_CAP));
    return recs.length;
  } catch {
    return 0;
  }
...
/** Load recent red-gate outputs (newest-first, capped). */
export function loadGateFailures(cwd, max = 20) {
  return readJsonStore(cwd, GATE_FAILURES_FILE, "records").slice(0, max);
}
...
export function recordGateFailure(cwd, { output }) {
  try {
    ...
    writeJsonStore(cwd, GATE_FAILURES_FILE, "records", recs.slice(0, GATE_FAILURES_CAP));
    return recs.length;
  } catch {
    return 0;
  }
